...
def get_duration(direction, tariff_id):
    (usecs, ) = db.query('select if(t.duration <> 0, t.duration, d.duration) from tariffs_tariff t \
                            left outer join tariffs_direction d on d.abbr="{0}" \
                            where t.id={1};'.format(direction, tariff_id)
                        )
